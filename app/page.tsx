...
export default async function Home() {
  ...
  try {
    ...
    error = "Failed to load podcast data"
  }

  return (
    <div className="bg-gradient-to-br from-purple-400 via-pink-500 to-red-500 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-4xl sm:text-5xl font-extrabold text-white text-center mb-4 drop-shadow-lg leading-tight">
          What Is This Place
        </h1>
        ...
        {/* Podcast Summary */}
        <div className="bg-white/10 backdrop-blur-sm rounded-lg p-6 mb-8 text-white text-center">
          <p className="text-lg leading-relaxed">{podcastData.podcastSummary}</p>
        </div>
        ...
        <div className="flex justify-center items-center space-x-6 mb-12">
          <Link
            ...
            className="flex items-center space-x-2 bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-full transition-all duration-300 backdrop-blur-sm"
          >
        ...
        {error ? <div className="text-red-500">{error}</div> : <PodcastFeed initialData={podcastData} />}
      </div>
    </div>
  )
}
